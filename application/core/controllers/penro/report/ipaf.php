...
class Ipaf extends CI_Controller
{
    ...
    public function byMonth()
    {
        ...
        $data['paList']     =   $this->penro_model->paList($provId);
        
        $data['content'] = $this->load->view('penro/report/ipaf_month',$data,TRUE);
    ...
    public function byYear(){
        ...
        $data['webSetting'] = $this->setting_model->webSetting();
        $data['read'] = $this->penro_model->read_by_id($user_id);
        $data['monthListed'] = $this->penro_model->monthListed();
        $data['yearListed']     = $this->penro_model->yearListed();
        $provId         =   $this->session->userdata('province');
        $data['paList']         = $this->penro_model->paList($provId);

        $data['content'] = $this->load->view('penro/report/ipaf_year',$data,TRUE);
        $this->load->view('penro/penro_wrapper',$data);
    }
    ...
    public function incomeSearch()
    {
        ...
        $user_id = $this->session->userdata('user_id');
        $data = $this->penro_model->getallfeeReportQuerried($searchPa,$year_list,$month_list,$user_id);
        foreach($data as $row){
            ?>
            <tr>
    ...
    public function incomeSearchYear()
    {
        $searchPa   = $this->input->post('searchPa');
        $year_list  = $this->input->post('year_list');
        $data = $this->penro_model->getIpafYearPenro($searchPa,$year_list);
        foreach($data as $row){
            ?>
            <tr>
